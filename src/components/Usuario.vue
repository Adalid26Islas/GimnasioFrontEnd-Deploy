<template>
        ...
        <div class="lg:w-1/2 xl:w-5/12 p-6 sm:p-12">
          <div class="mt-24 ml-20 flex flex-col items-center">
            ...
            <div class="w-full flex-1 mt-8">
              <div class="w-full">
                <label for="username" class="block text-gray-100 mt-8">Nombre de Usuario</label>
                <input type="text" required id="username" v-model="user.username"
                       class="w-full border-gray-300 bg-gray-800 text-gray-100 rounded-md py-2 px-3 focus:outline-none focus:border-blue-500">
  
                <label for="Numero_Telefononico_Movil" class="block text-gray-100 mt-8">Número de Teléfono</label>
                <input type="Numero_Telefononico_Movil" required id="Numero_Telefononico_Movil" v-model="user.Numero_Telefononico_Movil"
                       class="w-full border-gray-300 bg-gray-800 text-gray-100 rounded-md py-2 px-3 focus:outline-none focus:border-blue-500">
  
                <label for="Correo_Electronico" class="block text-gray-100 mt-8 mb-2">Correo Electrónico</label>
                <input type="Correo_Electronico" required id="Correo_Electronico" v-model="user.Correo_Electronico"
                       class="w-full border-gray-300 bg-gray-800 text-gray-100 rounded-md py-2 px-3 focus:outline-none focus:border-blue-500" >
  
                <label for="Contrasena" class="block text-gray-100 mt-8">Contraseña</label>
                <input type="Contrasena" required id="Contrasena" v-model="user.Contrasena"
                       class="w-full border-gray-300 bg-gray-800 text-gray-100 rounded-md py-2 px-3 focus:outline-none focus:border-blue-500" >
  ...
  export default {
    ...
    methods: {
      async login() {
        if (!this.user.username || !this.user.Correo_Electronico || !this.user.Numero_Telefononico_Movil || !this.user.Contrasena) {
        ...
        try {
          ...
          const response = await apiClient.post('/',JSON.stringify(this.user)
          );          
